...
generate_ai_pr_prompt() {
    ...
    echo "生成PR標題和內容，格式：標題|||內容。Issue: $issue_key, 分支: $branch_name, 變更: $commits $file_changes。要求：標題必須極度簡潔，僅10-20字描述核心功能（如：新增用戶登入功能、修復支付流程錯誤、優化資料庫查詢效能）。內容必須詳細包含：1)主要功能變更和影響範圍 2)技術實作細節和架構調整 3)新增或修改的關鍵檔案 4)相關的測試驗證 5)對使用者體驗的改善。使用繁體中文，內容結構化且具體。"
}
...
- [ ] 無破壞性變更"
            ...
            # 為較長內容添加完整的結構化格式
            # 檢查是否已經包含結構化內容
            if [[ ! "$body" =~ (功能變更|技術實作|檔案變更|測試|使用者) ]]; then
                body="## 📝 變更說明
...
execute_create_pr() {
    ...
    if [ -z "$pr_body" ]; then
        echo >&2
        info_msg "💡 建議包含以下內容：功能變更、技術細節、檔案變更、測試驗證、使用者影響" >&2
        printf "請輸入 PR 詳細描述 (可選，直接按 Enter 跳過): " >&2
