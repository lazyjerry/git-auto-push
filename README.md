...
4. **選擇模式**：用戶選擇要執行的操作範圍
5. **生成訊息**：智能 AI 生成或手動輸入 commit message（需要時）
6. **確認提交**：顯示提交資訊供用戶確認（需要時）
7. **執行操作**：根據選擇執行對應的 Git 操作
...
| 工具       | 說明                 | 優先級      |
| ---------- | -------------------- | ----------- |
| **gemini** | Google Gemini CLI    | 🥇 第一優先 |
| **codex**  | GitHub Copilot CLI   | 🥈 第二優先 |
...
可以修改 `ai_tools` 陣列來調整 AI 工具的使用順序：
...
請輸入選項 [1-3] (直接按 Enter 使用預設選項 1): 3
