...
# 作者：Lazy Jerry | 版本：v2.7.0 | 授權：MIT License
...
run_copilot_command() {
    ...
    if command -v timeout >/dev/null 2>&1; then
        output=$(run_command_with_loading "LC_ALL=en_US.UTF-8 timeout ${timeout}s copilot -s -p \"\$(cat '$temp_prompt')\" 2>&1" "正在等待 copilot 分析內容" "$timeout")
        exit_code=$?
    else
        output=$(run_command_with_loading "LC_ALL=en_US.UTF-8 copilot -s -p \"\$(cat '$temp_prompt')\" 2>&1" "正在等待 copilot 分析內容" "$timeout")
        exit_code=$?
    fi
